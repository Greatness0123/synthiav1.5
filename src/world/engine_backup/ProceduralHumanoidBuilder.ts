/**
 * ProceduralHumanoidBuilder — Three.js Primitive Model via Ragdoll System
 *
 * Creates a humanoid character from Three.js primitive shapes (spheres,
 * capsules, boxes) with matching Rapier rigid bodies, impulse joints,
 * and PD motor control. Routed through the same HumanoidMultiBodyManager
 * infrastructure as the x-bot.glb rigged model.
 *
 * From ProtoMotion extraction (Module 3):
 *   - Head: Sphere
 *   - Torso, Pelvis, Limbs: Capsules
 *   - Hands, Feet: Boxes
 *   - Mass distribution: heavy torso (low CoG), light limbs
 *
 * Usage:
 *   const builder = new ProceduralHumanoidBuilder(scene, physicsEngine);
 *   const result = builder.build(spawnPoint);
 *   // result.mesh → add to scene
 *   // result.bodies → pass to HumanoidMultiBodyManager
 *   // result.boneMap → for AvatarSynchronizer registration
 */
...
// Each part defines: shape, dimensions, density, position relative to root,
// parent connection, joint axis, and joint limits.
interface BodyPartDef {
  ...
  jointAxis?: [number, number, number]; // for revolute
  jointLimits?: [number, number]; // for revolute [min, max]
}

// ProtoMotion anthropometric body layout (Y-up, meters)
// Position offsets relative to the pelvis root at (0, 0.9, 0)
...
// ── Material for procedural body parts ──
...
export class ProceduralHumanoidBuilder {
  ...
  /**
   * Build the complete procedural humanoid at the given spawn position.
   * Returns the visual group, physics bodies, and mapping data.
   */
  public build(spawnPoint: THREE.Vector3): ProceduralBuildResult {
    const rootGroup = new THREE.Group();
    rootGroup.position.copy(spawnPoint);
    rootGroup.position.y += 0.78; // Offset so feet rest at ground level (lowest part is -0.78 from pelvis)
    ...
    // Build parts in order (parents before children)
    for (const def of BODY_PARTS) {
      const parentPart = def.parent ? parts.get(def.parent) : null;
      const parentBody = parentPart?.body ?? null;

      // World position = spawn + local offset
      const feetOffset = 0.78; // Offset pelvis up so feet rest at ground level
      ...
      // 4. Create impulse joint to parent
      ...
      // Store reference on mesh for raycasting/selection
      mesh.userData.proceduralPart = def.name;
      mesh.userData.isSynthiaPrimitive = true;
    }

    // Calculate actual height: from lowest part (ankle at -0.78) to highest (head at +0.95 + sphere radius 0.10)
    const lowestY = Math.min(...BODY_PARTS.map(p => p.localPos[1]));
    const highestY = Math.max(...BODY_PARTS.map(p => p.localPos[1]));
    const totalHeight = highestY - lowestY + 0.10; // +0.10 for head sphere radius
  ...
  /**
   * Create a Three.js mesh for a body part.
   */
  ...
  /**
   * Create a Rapier collider descriptor matching the body part shape.
   */
  ...
  /**
   * Sync visual mesh positions from physics bodies.
  ...
  public syncVisuals(parts: Map<string, ProceduralPart>): void {
    parts.forEach((part) => {
      ...
      // Convert world-space physics position to local position relative to rootGroup
  ...
  /**
   * Remove all procedural parts from the scene and physics world.
   */
