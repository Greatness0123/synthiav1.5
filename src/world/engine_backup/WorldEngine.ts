/**
 * Core Three.js scene manager, handling rendering and the main animation loop.
 */
...
export class WorldEngine {
  ...
  // Offscreen AI frame (448×448 WebP base64, captured each animation frame)
  ...
  private setupLights(): void {
    // Global ambient bounce light so shadows aren't pitch black
    this.ambientLight = new THREE.AmbientLight(0xffffff, 0.6);
    this.scene.add(this.ambientLight);

    // Primary directional sun light to throw structural shadows
  ...
  private setupEnvironment(): void {
    // Absolute background color for visual fallback (Sky)
    ...
    this.floorMesh.position.y = -0.01; // Slightly below y=0 to avoid z-fighting with objects resting on 0
  ...
  private onPointerUp = (event: PointerEvent): void => {
    ...
    // Calculate how far the mouse was dragged between down and up
    const movementDistance = this.pointerDownPosition.distanceTo(new THREE.Vector2(event.clientX, event.clientY));
    if (movementDistance > 5) {
      return; // Ignore if user was dragging (e.g., camera orbiting)
    }
    ...
    // If the user clicked on the TransformControls gizmo helper itself,
    // bail out early so we don't accidentally detach the selection.
  ...
  public start(onStep?: () => void): void {
    ...
    const animate = (time: number) => {
      ...
      // Reset accumulator on false→true transition to prevent burst of
      // multiple physics steps in a single frame after isReady re-enables.
      ...
      // onStep already calls cameraManager.update(headMatrix, position) for humanoid mode.
      // Do NOT call cameraManager.update() here — it would overwrite the head-locked camera
      // with no-args (falling into the else branch), causing a one-frame flicker.

      // Capture AI frame (448×448 WebP) from the dedicated render target
      try {
      ...
      // Use active camera from manager and render main view to screen
      this.camera = this.cameraManager.getMainCamera();
  ...
  /**
   * Get the most recently captured AI perception frame (448×448 WebP base64, no prefix).
   * Updated every animation frame by captureAIFrame().
   */
