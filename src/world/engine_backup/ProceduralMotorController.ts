/**
 * ProceduralMotorController — PD Motor Control for ProceduralHumanoidBuilder
 *
 * Applies PD motor control to the procedural model's impulse joints.
 * Uses Rapier's built-in PD motor for revolute joints and manual PD torque
 * for spherical joints (same approach as HumanoidMultiBodyManager).
 *
 * Integrates with the AI action pipeline via the same canonical bone
 * name convention used by HumanoidPhysicsBinder.
 */
...
// PD gain presets for procedural model joints
interface PDGains {
  stiffness: number;
  damping: number;
}

// Gains tuned for the procedural model's body proportions
...
// Joint type per procedural body part
...
// Hierarchy for computing relative rotations in manual PD
...
export class ProceduralMotorController {
  ...
  /**
   * Initialize from a ProceduralBuildResult.
   * Registers all joints with the underlying motor controller.
   */
  public init(buildResult: ProceduralBuildResult): void {
    ...
    // Set all targets to zero (bind pose)
  ...
  /**
   * Set motor targets from the AI action pipeline's currentTargets map.
   * Same interface as HumanoidMultiBodyManager.setTargets().
   */
  public setTargets(currentTargets: Map<string, any>): void {
    currentTargets.forEach((parsedTarget, canonical) => {
      ...
      if (jointType === 'revolute' && joint) {
        // Revolute joint: use Rapier built-in PD motor
        let targetAngle = 0;
        ...
        // Spherical joint: quaternion axis-angle PD torque (avoids Euler gimbal lock)
        ...
        // Compute current relative rotation (parent frame → child frame)
  ...
  /**
   * Set a single joint's target angle (for direct control).
   */
  ...
  /**
   * Reset all targets to zero (standing pose).
   */
