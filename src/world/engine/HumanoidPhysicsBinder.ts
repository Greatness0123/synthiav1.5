...
import { MJCFAgentSpec } from './MJCFHumanoidTemplate';
...
interface AgentVisual {
  ...
  currentTargets: Map<string, any>;
  timelineQueue: TimelineSequence;
  timelineSequenceStart: number | null;
  groundSurfaceY: number;
  isGrounded: boolean;
  targetSpawnGrounded: boolean;
  previousFootPositions: Map<string, THREE.Vector3>;
}

export class HumanoidPhysicsBinder {
  private physicsEngine: PhysicsEngine;
  private scene: THREE.Scene;
  private baseGltfScene: THREE.Group | null = null;
  private baseBoneInfoMap: Map<string, BoneInfo> = new Map();
  private baseSkeleton: THREE.Skeleton | null = null;
  private baseSkinnedMesh: THREE.SkinnedMesh | null = null;
  ...
  public getAgents(): Map<string, AgentVisual> {
    return this.agents;
  }

  private resolveAgentId(id?: string): string {
    if (id) return id;
    const firstId = this.agents.keys().next().value;
    return firstId || 'agent_0';
  }

  public validateAndApplyTimeline(targetSkeleton: THREE.Skeleton, sequence: TimelineSequence, options?: { activeGaitPhase?: boolean }, agentId: string = ''): ValidateResult {
    ...
    const targetId = this.resolveAgentId(agentId);
    const agent = this.agents.get(targetId);
    if (!agent) {
      return { appliedTimeline: [], rejections: [`unknown_agent:${targetId}`], clampingNotes: [], injections: [] };
    }
        ...
        if (constraint.dof === 1) {
          ...
          if (constraint.allowance?.tendonSynergyLink) {
            ...
            if (!baseOverrideInFrame) {
              const baseTarget = agent.currentTargets.get(baseKey) as any;
    ...
    agent.timelineQueue = appliedTimeline;
  ...
  public async loadAndVisualizeBindPose(spawnPoint: THREE.Vector3): Promise<boolean> {
    ...
    try {
      ...
      this.baseGltfScene = gltf.scene;

      this.baseGltfScene!.traverse((child) => {
        if ((child as any).isSkinnedMesh) {
          this.baseSkinnedMesh = child as THREE.SkinnedMesh;
        }
      });

      this.baseSkeleton = this.baseSkinnedMesh!.skeleton;

      this.baseSkeleton!.bones.forEach((bone) => {
        const canonicalName = bone.name.toLowerCase().replace(/:/g, '');
        const worldPos = new THREE.Vector3();
        bone.getWorldPosition(worldPos);
        this.baseBoneInfoMap.set(canonicalName, {
          bone,
      ...
      await this.spawnAgentVisual('agent_0', spawnPoint);

      this.buildStep = 'A';
      this.isLoaded = true;
      this.physicsEngine.setReady(true);

      Logger.info(`HumanoidPhysicsBinderMuJoCo STEP A Complete: Loaded template bones.`);
      return true;
    } catch (error) {
  ...
  public async spawnAgentVisual(id: string, spawnOffset: THREE.Vector3): Promise<void> {
    if (!this.baseGltfScene) return;

    const clonedScene = this.baseGltfScene.clone();
    clonedScene.userData.isSynthiaPrimitive = true;
    clonedScene.userData.agentId = id;
    this.scene.add(clonedScene);
    clonedScene.position.copy(spawnOffset);
    ...
    const boneInfoMap = new Map<string, BoneInfo>();

    skeleton.bones.forEach((bone) => {
      const canonicalName = bone.name.toLowerCase().replace(/:/g, '');
      const worldPos = new THREE.Vector3();
      bone.getWorldPosition(worldPos);
      boneInfoMap.set(canonicalName, {
        bone,
        worldPosition: worldPos.clone(),
        name: canonicalName,
      });
    ...
    const agent: AgentVisual = {
      id,
      modelRoot: clonedScene,
      skeleton,
      skinnedMesh: skinnedMesh!,
      boneInfoMap,
      currentTargets: new Map(),
      timelineQueue: [],
      timelineSequenceStart: null,
      groundSurfaceY: 0.0,
      isGrounded: true,
      targetSpawnGrounded: false,
      previousFootPositions: new Map()
    };

    this.agents.set(id, agent);
    clonedScene.updateMatrixWorld(true);

    this.resetToBindPoseForAgent(id);
    console.log(`[HumanoidPhysicsBinder] Spawned visual agent ${id} at offset x=${spawnOffset.x}, y=${spawnOffset.y}, z=${spawnOffset.z}`);
  }

  public async spawnAgent(id: string, spawnOffset: THREE.Vector3): Promise<void> {
    this.physicsEngine.setMutating(true);
    this.physicsEngine.setReady(false);

    try {
      await this.spawnAgentVisual(id, spawnOffset);

      const specList: MJCFAgentSpec[] = [];
      this.agents.forEach((agent, agentId) => {
        specList.push({
          id: agentId,
          spawnOffset: {
            x: agent.modelRoot.position.x,
            y: agent.modelRoot.position.y,
            z: agent.modelRoot.position.z
          }
        });
      });
      ...
      const success = await this.bodyManager.activate(
        this.baseBoneInfoMap,
        this.baseSkeleton!,
        null,
        this.capsuleCenterY,
        this.agents.get('agent_0')!.modelRoot,
        specList
      );
      ...
      if (success) {
        ...
        this.agents.forEach((activeAgent, activeId) => {
          this.bodyManager.syncRigidBodiesFromBones(activeAgent.boneInfoMap, activeId, activeAgent.modelRoot);
        });

        await this.activateMultiBody();
      }

      this.physicsEngine.setReady(true);
    } catch (err) {
      console.error('[HumanoidPhysicsBinder] spawnAgent physics crash:', err);
    } finally {
      this.physicsEngine.setMutating(false);
    }
  }
  ...
  private calculateModelDimensions(): void {
    ...
    this.baseBoneInfoMap.forEach((info) => {
    ...
    this.baseBoneInfoMap.forEach((info, name) => {
  ...
  public repositionModel(x: number, y: number, z: number, agentId: string = ''): void {
    const targetId = this.resolveAgentId(agentId);
    const agent = this.agents.get(targetId);
    if (!agent) return;

    agent.modelRoot.position.set(x, y, z);
    agent.modelRoot.updateMatrixWorld(true);

    agent.boneInfoMap.forEach((info) => {
      const worldPos = new THREE.Vector3();
  ...
  public renderDebugSpheres(show: boolean = false): void {
    ...
    this.agents.forEach((agent) => {
      agent.boneInfoMap.forEach((boneInfo, boneName) => {
        ...
        this.debugSpheres.set(agent.id + '_' + boneName, sphere);
      });
    });
  }
  ...
  public async activateMultiBody(): Promise<boolean> {
    if (this.buildStep !== 'D' || !this.baseSkeleton) return false;
    ...
    try {
      ...
      const agent_0 = this.agents.get('agent_0')!;
      const capsuleBodyId = this.bodyManager.getCapsuleBody('agent_0');
      ...
      if (capsuleBodyId !== null && capsuleBodyId >= 0) {
        ...
        for (const [canonical, bodyId] of bodyIds) {
          if (canonical.startsWith('agent_0_') && canonical !== 'agent_0_root_capsule') {
            const unPrefixed = canonical.replace('agent_0_', '');
      ...
      this.agents.forEach((agent, id) => {
        const prefix = id ? `agent_${id}_` : '';
        for (const [canonical] of bodyIds) {
          if (canonical.startsWith(prefix) && canonical !== prefix + 'root_capsule') {
            const unPrefixed = canonical.replace(prefix, '');
            this.avatarSynchronizer.registerBone(canonical, agent.boneInfoMap.get(unPrefixed)!.bone.name, {
              canonicalName: canonical, syncRotation: true, syncTranslation: false, rootOffsetY: 0,
            });
          }
        }
      });
  ...
  public syncVisuals(): void {
    ...
    this.agents.forEach((agent, id) => {
      const capsuleBodyId = this.bodyManager.getCapsuleBody(id);
      ...
      agent.modelRoot.position.copy(capsulePosition).sub(offsetWorld);
      agent.modelRoot.quaternion.copy(capsuleQuaternion);
      ...
      const prefix = id ? `agent_${id}_` : '';
      const capsuleGeomId = this.bodyManager.getBoneColliderHandle(prefix + 'root_capsule') ?? -1;
      const dist = module.mj_ray(model, data, capsulePosMj, downDirMj, geomgroup, true, capsuleGeomId, geomIdBuffer, null);

      if (dist >= 0) {
        agent.groundSurfaceY = t.y - dist;
      } else {
        agent.groundSurfaceY = 0.0;
      }
      geomIdBuffer.delete();

      if (!agent.targetSpawnGrounded && dist >= 0) {
        let lowestFootY = Infinity;
        for (const [, info] of agent.boneInfoMap) {
          if (info.name.includes('foot') || info.name.includes('toe')) {
            const worldPos = new THREE.Vector3();
            info.bone.getWorldPosition(worldPos);
        ...
        if (lowestFootY < Infinity) {
          const delta = agent.groundSurfaceY - lowestFootY;
          if (Math.abs(delta) > 0.001) {
            this.setCapsulePosition(t.x, t.y + delta - this.capsuleCenterY, t.z, id);
          }
        }
        agent.targetSpawnGrounded = true;
      }

      const capsuleBottomY = t.y - this.capsuleCenterY;
      agent.isGrounded = capsuleBottomY <= (agent.groundSurfaceY + this.GROUND_SNAP_THRESHOLD);

      this.applyKinematicGroundReactionForces(id);
      ...
      if (this.mbActive) {
        ...
        for (const [canonical, bodyId] of this.bodyManager.getRigidBodiesMap()) {
          if (canonical.startsWith(prefix) && canonical !== prefix + 'root_capsule') {
            const unPrefixed = canonical.replace(prefix, '');
            const boneInfo = agent.boneInfoMap.get(unPrefixed);
      ...
      if (agent.timelineQueue.length > 0) {
        if (agent.timelineSequenceStart === null) {
          agent.timelineSequenceStart = Date.now();
        }

        const elapsed = Date.now() - agent.timelineSequenceStart;
        const sorted = agent.timelineQueue.slice().sort((a, b) => a.timeOffsetMs - b.timeOffsetMs);
        ...
        if (activeIdx >= 0) {
          ...
          if (nextFrame) {
            ...
            this.setMotorTargets(interpolatedOverrides as any, id);
          } else {
            this.setMotorTargets(activeFrame.overrides as any, id);
          }
        }

        const GRACE_MS = 50;
        agent.timelineQueue = sorted.filter(f => (f.timeOffsetMs || 0) > elapsed - GRACE_MS);
        if (agent.timelineQueue.length === 0) agent.timelineSequenceStart = null;
      }
    });

    if (this.debugSpheres.size > 0) {
      this.agents.forEach((agent) => {
        agent.boneInfoMap.forEach((boneInfo, boneName) => {
          const debugSphere = this.debugSpheres.get(agent.id + '_' + boneName);
          if (debugSphere) {
            const worldPos = new THREE.Vector3();
            boneInfo.bone.getWorldPosition(worldPos);
  ...
  private applyKinematicGroundReactionForces(agentId: string): void {
    const agent = this.agents.get(agentId);
    if (!agent) return;

    const capsuleBodyId = this.bodyManager.getCapsuleBody(agentId);
    if (capsuleBodyId === null || capsuleBodyId < 0) return;
    ...
    const modelQuat = agent.modelRoot.quaternion.clone();

    feetNames.forEach((boneName) => {
      const boneInfo = agent.boneInfoMap.get(boneName);
      if (!boneInfo) return;

      const currentPos = new THREE.Vector3();
      boneInfo.bone.getWorldPosition(currentPos);

      const previousPos = agent.previousFootPositions.get(boneName);
      if (previousPos) {
        if (currentPos.y <= agent.groundSurfaceY + 0.15) {
          ...
          if (deltaMag > MAX_POSE_FOOT_DELTA) {
            agent.previousFootPositions.set(boneName, currentPos.clone());
            return;
          }
  ...
  public getIsGrounded(agentId: string = ''): boolean {
    const targetId = this.resolveAgentId(agentId);
    return this.agents.get(targetId)?.isGrounded ?? true;
  }

  public executeJump(force: number = 6.0, agentId: string = ''): void {
    const targetId = this.resolveAgentId(agentId);
    const agent = this.agents.get(targetId);
    if (!agent) return;

    const capsuleBodyId = this.bodyManager.getCapsuleBody(targetId);
    if (capsuleBodyId === null || capsuleBodyId < 0) return;
    ...
    Logger.info(`HumanoidPhysicsBinderMuJoCo.executeJump: Jump impulse Z=${deltaV} applied for ${targetId}.`);
  }

  public setBoneRotation(boneName: string, quaternion: THREE.Quaternion, agentId: string = ''): void {
    const targetId = this.resolveAgentId(agentId);
    const agent = this.agents.get(targetId);
    if (!agent) return;
    const boneInfo = agent.boneInfoMap.get(boneName.toLowerCase().replace(/:/g, ''));
    if (!boneInfo) return;
    boneInfo.bone.quaternion.copy(quaternion);
  }

  public setCapsulePosition(x: number, y: number, z: number, agentId: string = ''): void {
    const targetId = this.resolveAgentId(agentId);
    const capsuleBodyId = this.bodyManager.getCapsuleBody(targetId);
    if (capsuleBodyId === null || capsuleBodyId < 0) return;
    ...
    const prefix = targetId ? `agent_${targetId}_` : '';
  ...
  public getJointState(agentId: string = ''): Record<string, { position: [number, number, number], rotation: [number, number, number, number] }> {
  ...
  public getHeadTransform(agentId: string = ''): { position: THREE.Vector3, quaternion: THREE.Quaternion } | null {
    const targetId = this.resolveAgentId(agentId);
    const agent = this.agents.get(targetId);
    if (!agent) return null;

    let headBone: THREE.Bone | null = null;
    for (const [name, info] of agent.boneInfoMap) {
  ...
  public getContactForces(agentId: string = ''): Record<string, { contact: boolean; impulse_magnitude: number; contact_normal: [number, number, number]; touching: string }> {
    const result: Record<string, { contact: boolean; impulse_magnitude: number; contact_normal: [number, number, number]; touching: string }> = {};

    const targetId = this.resolveAgentId(agentId);
    const prefix = targetId ? `agent_${targetId}_` : '';
    const capsuleGeomId = this.bodyManager.getBoneColliderHandle(prefix + 'root_capsule');
  ...
  public push(partName: string, impulse: THREE.Vector3, agentId: string = ''): void {
    const targetId = this.resolveAgentId(agentId);
    const capsuleBodyId = this.bodyManager.getCapsuleBody(targetId);
    if (capsuleBodyId === null || capsuleBodyId < 0) return;
    ...
    Logger.info(`HumanoidPhysicsBinderMuJoCo: Applied push velocity change for ${targetId}: [${deltaV.x.toFixed(2)}, ${deltaV.y.toFixed(2)}, ${deltaV.z.toFixed(2)}]`);
  }
  ...
  public getUprightPreset(agentId: string = ''): Record<string, any> {
    ...
    const agent = this.agents.get(targetId);
    if (agent) {
      agent.currentTargets.forEach((val, key) => {
  ...
  public setMotorTargets(targets: Record<string, number | number[]>, agentId: string = ''): ActionApplyResult {
    ...
    const targetId = this.resolveAgentId(agentId);
    const agent = this.agents.get(targetId);
    if (!agent) return { applied, rejected };
    ...
    for (const [boneName, target] of Object.entries(targets)) {
      ...
      if (!agent.boneInfoMap.has(canonical)) {
        rejected.push({
          joint: boneName,
      ...
      agent.currentTargets.set(canonical, parsedTarget);
  ...
  public updateMotorTargets(): void {
    ...
    // Accumulate all targets as prefixed entries for the motor controller
    const prefixedTargets = new Map<string, any>();
    this.agents.forEach((agent, id) => {
      const prefix = id ? `agent_${id}_` : '';
      agent.currentTargets.forEach((val, key) => {
        prefixedTargets.set(prefix + key, val);
      });
    });

    this.motorController.setTargets(prefixedTargets);

    // Balance each agent independently
    this.agents.forEach((agent, id) => {
      const capsuleBodyId = this.bodyManager.getCapsuleBody(id);
      if (capsuleBodyId !== null && capsuleBodyId >= 0) {
  ...
  public executeProgramSequence(programs: string[], agentId: string = ''): void {
    const targetId = this.resolveAgentId(agentId);
    const capsuleBodyId = this.bodyManager.getCapsuleBody(targetId);
    if (capsuleBodyId === null || capsuleBodyId < 0) return;
    ...
    for (const program of programs) {
      ...
      if (name.includes('stand') || name.includes('upright') || name.includes('recover') || name.includes('reorient')) {
        const agent = this.agents.get(targetId);
        if (agent) {
          const spawnOffset = agent.modelRoot.position;
          this.setCapsulePosition(spawnOffset.x, spawnOffset.y, spawnOffset.z, targetId);
        }
        this.resetToBindPoseForAgent(targetId);
      } else if (name.includes('jump')) {
        this.executeJump(6.0, targetId);
      } else {
        Logger.warn(`HumanoidPhysicsBinder: Unknown program sequence "${program}" ignored for ${targetId}.`);
      }
    }
  }

  public resetPose(spawnPoint: { x: number; y: number; z: number }, agentId: string = ''): void {
    const targetId = this.resolveAgentId(agentId);
    this.setCapsulePosition(spawnPoint.x, spawnPoint.y, spawnPoint.z, targetId);
    this.resetToBindPoseForAgent(targetId);
    this.agents.get(targetId)?.previousFootPositions.clear();
  }

  public isOutOfWorldBounds(agentId: string = ''): boolean {
    const targetId = this.resolveAgentId(agentId);
    const capsuleBodyId = this.bodyManager.getCapsuleBody(targetId);
  ...
  public resetToBindPoseForAgent(agentId: string): void {
    const agent = this.agents.get(agentId);
    if (!agent) return;

    agent.currentTargets.clear();
    ...
    // Reset all hinge qpos values to 0
    for (const [boneName] of this.baseBoneInfoMap) {
    ...
    agent.currentTargets.set('mixamorigrightarm', { x: armsDownAngle, y: 0, z: 0, isQuaternion: false });
    agent.currentTargets.set('mixamorigleftarm', { x: armsDownAngle, y: 0, z: 0, isQuaternion: false });
    agent.currentTargets.set('mixamorigspine', { x: 0, y: 0, z: 0, isQuaternion: false });
    agent.currentTargets.set('mixamorigspine1', { x: 0, y: 0, z: 0, isQuaternion: false });
    agent.currentTargets.set('mixamorigspine2', { x: 0, y: 0, z: 0, isQuaternion: false });
    agent.currentTargets.set('mixamorigleftupleg', { x: 0, y: 0, z: 0, isQuaternion: false });
    agent.currentTargets.set('mixamorigrightupleg', { x: 0, y: 0, z: 0, isQuaternion: false });
    agent.currentTargets.set('mixamorigleftleg', { x: 0, y: 0, z: 0, isQuaternion: false });
    agent.currentTargets.set('mixamorigrightleg', { x: 0, y: 0, z: 0, isQuaternion: false });
    agent.currentTargets.set('mixamorigleftfoot', { x: 0, y: 0, z: 0, isQuaternion: false });
    agent.currentTargets.set('mixamorigrightfoot', { x: 0, y: 0, z: 0, isQuaternion: false });
  }
  ...
  public getModelRoot(agentId: string = ''): THREE.Group | null {
    const targetId = this.resolveAgentId(agentId);
    return this.agents.get(targetId)?.modelRoot ?? null;
  }

  public getCapsuleBody(agentId: string = ''): any {
    const targetId = this.resolveAgentId(agentId);
    const capsuleBodyId = this.bodyManager.getCapsuleBody(targetId);
  ...
  public getDiagnostics(): Record<string, any> {
    ...
    return {
      ...
      boneCount: this.baseBoneInfoMap.size,
  ...
  public cleanup(): void {
    ...
    this.agents.forEach((agent) => {
      this.scene.remove(agent.modelRoot);
      agent.modelRoot.traverse((child) => {
    ...
    this.baseBoneInfoMap.clear();
    this.baseSkeleton = null;
    this.baseSkinnedMesh = null;
    this.baseGltfScene = null;
    this.isLoaded = false;
    this.buildStep = null;
    this.mbActive = false;
  }
