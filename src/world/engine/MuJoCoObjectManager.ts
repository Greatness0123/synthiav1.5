...
export class MuJoCoObjectManager {
  ...
  constructor(physicsEngine: MuJoCoPhysicsEngine, scene: THREE.Scene, audioEngine: AudioEngine) {
  ...
  private reloadStateAndRehydrate(newMeshSpec?: any) {
    ...
    try {
      ...
      const customModelsXml = this.customMeshesSpec.map((spec) => {
        const posMj = MuJoCoPhysicsEngine.worldToMuJoCo(spec.position);
        const quatMj = spec.quaternion
          ? MuJoCoPhysicsEngine.threeQuatToMuJoCo(spec.quaternion)
          : [1, 0, 0, 0];
    ...
    <body name="custom_${spec.id}" pos="${posMj[0]} ${posMj[1]} ${posMj[2]}" quat="${quatMj[0]} ${quatMj[1]} ${quatMj[2]} ${quatMj[3]}">
      ...
      this.objects.forEach((obj) => {
        ...
        if (bodyId >= 0) {
          ...
          if (obj.isCustom) {
            ...
            // Re-claim sibling geom ID (mapping wedge, slope, ramp to box)
            const actualPresetShapeId = ['wedge', 'slope', 'ramp'].includes(obj.preset.id) ? 'box' : obj.preset.id;
      ...
      Logger.error('MuJoCoObjectManager: Mesh reload and state hydration failed!', error);
    } finally {
  ...
  public spawnObject(presetId: string, position: THREE.Vector3): WorldObject | null {
    ...
    if (slotIdx < 0) {
      Logger.warn('MuJoCoObjectManager: Pre-allocated slots exhausted (all 20 slots active!)');
      return null;
    }
    ...
    // We map wedge, slope, ramp to oriented dynamic box geoms as confirmed by the user
    const actualPresetShapeId = ['wedge', 'slope', 'ramp'].includes(preset.id) ? 'box' : preset.id;
    ...
    if (geomId >= 0) {
      ...
      const adapterGeom = MuJoCoCollisionAdapter.objectPresetToMJCFGeom(preset);
    ...
    const posMj = MuJoCoPhysicsEngine.worldToMuJoCo(position);
  ...
  public setObjectPosition(id: string, position: THREE.Vector3, quaternion?: THREE.Quaternion): void {
    ...
    if (quaternion) {
      const quatMj = MuJoCoPhysicsEngine.threeQuatToMuJoCo(quaternion);
  ...
  public syncVisuals() {
    ...
    this.objects.forEach((obj) => {
      ...
      if (obj.id === this.draggingObjectId) {
        ...
        const posMj = MuJoCoPhysicsEngine.worldToMuJoCo(obj.mesh.position);
        data.qpos[qposAdr] = posMj[0];
        data.qpos[qposAdr + 1] = posMj[1];
        data.qpos[qposAdr + 2] = posMj[2];

        const quatMj = MuJoCoPhysicsEngine.threeQuatToMuJoCo(obj.mesh.quaternion);
        data.qpos[qposAdr + 3] = quatMj[0];
        data.qpos[qposAdr + 4] = quatMj[1];
        data.qpos[qposAdr + 5] = quatMj[2];
        data.qpos[qposAdr + 6] = quatMj[3];
        ...
        const posThree = MuJoCoPhysicsEngine.mujocoToWorld([x_mj, y_mj, z_mj]);
        ...
        const rotThree = MuJoCoPhysicsEngine.mujocoQuatToThree([qW, qX, qY, qZ]);
  ...
  public update() {
    ...
    const world = this.physicsEngine.getWorld();
    const pairs = MuJoCoCollisionAdapter.getCollisionPairs(module, world.model, world.data);
