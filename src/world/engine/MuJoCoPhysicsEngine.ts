...
export class MuJoCoPhysicsEngine {
  ...
  private static async ensureMuJoCoInitialized(): Promise<MainModule> {
    if (MuJoCoPhysicsEngine.mujocoModule) return MuJoCoPhysicsEngine.mujocoModule;

    if (!MuJoCoPhysicsEngine.mujocoInitPromise) {
      MuJoCoPhysicsEngine.mujocoInitPromise = (async () => {
        ...
        MuJoCoPhysicsEngine.mujocoModule = module;
        return module;
      })();
    }

    try {
      return await MuJoCoPhysicsEngine.mujocoInitPromise;
    } catch (error) {
      MuJoCoPhysicsEngine.mujocoInitPromise = null;
      throw error;
    }
  }
  ...
  public async init(): Promise<void> {
    try {
      const module = await MuJoCoPhysicsEngine.ensureMuJoCoInitialized();
