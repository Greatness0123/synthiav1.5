...
class InferPayload(BaseModel):
    ...
    valid_joints: List[str] = []       # list of valid joint names
    ...
    class Config:
        extra = 'ignore'  # Tolerate any extra fields from coordinator
...
def build_prompt(payload: dict) -> list[dict]:
    ...
    motor_program_library = payload.get('motor_program_library')
    if not motor_program_library:
        motor_program_library = [
            "stand_upright", "step_forward", "step_backward",
            "step_left", "step_right", "turn_left", "turn_right",
            "get_up_from_front", "get_up_from_back"
        ]
...
program_sequence must only contain names from this exact list: {motor_program_library}.
IMPORTANT: gaze_target goes at the ROOT level of your response JSON, NOT inside the actions object. Example:
{{
  "memory_write": {{
  ...
  "actions": {{ "program_sequence": ["program_name"], "joint_overrides": {{}} }},
  "gaze_target": {{ "mode": "relative_angle", "value": [10, 0] }},
...
"""
    ...
    environmental_block = (
        ...
        "When you first begin a session, your starting pose is a T-pose (arms extended horizontally to your sides) — this is NOT your natural standing position, it is a default reference pose. "
        "Your first priority in a new session should be to move your arms down to your sides and settle into your upright_preset stance."
    )
    ...
    system_prompt = "\n\n".join(filter(None, [
        ...
        questioning_block, simultaneous_block, environmental_block,
...
def generate_stream(payload: InferPayload):
    ...
    if not payload.audio_pcm:
        clap_description = "No audio detected this cycle"
    else:
        try:
